...
const getPersonDisplayName = (record: any) => {
    ...
    return fullName || record.fullname || record.fullName || record.name || record.username || 'Unknown';
};

const getDisplayAppointmentStatus = (status: string | undefined, latestRescheduleRequest?: any) => {
    const baseStatus = (status || 'pending').toLowerCase();
    const requestStatus = (latestRescheduleRequest?.status || '').toLowerCase();

    if (['pending', 'needs_new_schedule'].includes(requestStatus) && !['completed', 'cancelled'].includes(baseStatus)) {
        return 'pending';
    }
...
const DECLINE_PREFERENCE_REASONS = [
    {
        id: 'slot_unavailable',
        label: 'Requested Slot No Longer Available',
        template: 'We reviewed your preferred date and time, but that requested slot is no longer available. Please wait for another schedule update from the clinic.'
    },
    {
        id: 'doctor_unavailable',
        label: 'Assigned Veterinarian Unavailable',
        template: 'We reviewed your preferred schedule, but the veterinarian needed for this appointment is unavailable at that requested time. Please wait for another schedule proposal from the clinic.'
    },
    {
        id: 'clinic_conflict',
        label: 'Clinic Schedule Conflict',
        template: 'We reviewed your preferred schedule, but the clinic has a scheduling conflict during that requested slot. Please wait for another schedule proposal from the clinic.'
    },
    {
        id: 'service_timing',
        label: 'Service Needs Different Timing',
        template: 'We reviewed your preferred schedule, but this appointment needs a different timing to properly accommodate the service. Please wait for another schedule proposal from the clinic.'
    },
    {
        id: 'please_wait',
        label: 'Please Wait for Another Proposal',
        template: 'We reviewed your preferred schedule, but we cannot approve it at this time. Please wait for another schedule proposal from the clinic.'
    },
    {
        id: 'specific',
...
const CustomCalendar = ({ selectedDate, onSelectDate, bookedDates = {}, availableDays = null, disablePastDates = false }: any) => {
    ...
    // Calculate the strict month boundaries
            ...
            days.push(
                <div 
                    ...
                    style={{
                        ...
                        fontSize: '13px', fontWeight: fontWeight,
    ...
    return (
        <div style={{ width: '100%', userSelect: 'none' }}>
            ...
            {/* Single Grid to perfectly align headers and numbers */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px 2px', justifyItems: 'center', minHeight: '200px' }}>
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} style={{ fontSize: '11px', color: '#a0a0a0', fontWeight: '700', marginBottom: '8px' }}>{day}</div>
                ))}
                {renderDays()}
            </div>
        </div>
...
const CreateAppointmentModal = ({ visible, onClose, onSubmit }: any) => {
    ...
    const medicalQuestionConfigs = [
        { key: 'medications72h', question: 'Were there any medications given to your pet in the past 72 hours?', detailLabel: 'Medication details' },
        { key: 'fleaPrevention', question: 'My pet has received up-to-date flea and tick prevention' },
        { key: 'catVaccinations', question: 'My cat has up-to-date anti rabies + 4 in 1' },
        { key: 'notPregnant', question: 'My pet is not pregnant' }
    ];
    const [medicalAnswers, setMedicalAnswers] = useState<Record<string, boolean | null>>(initialMedicalAnswers);
    ...
    const clearSelectedPetProfile = () => {
        resetFormState();
    };

    const getLockedFieldStyle = (extraStyles: any = {}) => (
        isProfileLocked
            ? { ...extraStyles, backgroundColor: '#f5f7fb', color: '#7a7a7a', cursor: 'not-allowed' }
            : extraStyles
    );
    ...
    const handleSave = () => {
        ...
        onSubmit({
            ...
            // Sending all reason aliases for safety
            ...
            medical_information: {
                ...
                additional_notes: medicalNotes.trim(),
                has_allergies: false,
                allergy_details: '',
                has_skin_condition: false,
                skin_condition_details: '',
    ...
    const filteredSearchList = fetchedPets.filter(pet => {
        const owner = fetchedPatients.find(p => p.id === pet.owner_id);
        const ownerName = owner ? getPersonDisplayName(owner) : '';
        const searchStr = `${pet.pet_name || ''} ${ownerName} ${owner?.username || ''}`.toLowerCase();
    ...
    return (
        <div className="modalOverlay" style={{ zIndex: 1000 }}>
            <div className="modalContainer" style={{ width: '95%', maxWidth: '600px', padding: '25px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                ...
                {isProfileLocked && (
                    <div style={{ marginBottom: '20px', padding: '10px 12px', borderRadius: '8px', backgroundColor: '#eef3ff', border: '1px solid #d6e2ff', color: '#3358c8', fontSize: '12px', lineHeight: '18px' }}>
                        This appointment is linked to an existing pet profile. Patient and pet profile fields are locked to match the saved record.
                    </div>
                )}
                ...
                <div style={{ overflowY: 'auto', paddingRight: '5px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="text" className="formInput" placeholder="First Name" maxLength={50} value={firstName} disabled={isProfileLocked} onChange={e => {setFirstName(e.target.value); setOwnerId('');}} style={getLockedFieldStyle()} />
                            </div>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="text" className="formInput" placeholder="Last Name" maxLength={50} value={lastName} disabled={isProfileLocked} onChange={e => {setLastName(e.target.value); setOwnerId('');}} style={getLockedFieldStyle()} />
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="email" className="formInput" placeholder="Enter email address" maxLength={60} value={email} disabled={isProfileLocked} onChange={e => setEmail(e.target.value)} style={getLockedFieldStyle()} />
                            </div>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="text" className="formInput" placeholder="0917-123-4567" maxLength={13} value={phone} disabled={isProfileLocked} onChange={e => setPhone(e.target.value)} style={getLockedFieldStyle()} />
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="text" className="formInput" placeholder="Enter pet name" maxLength={30} value={petName} disabled={isProfileLocked} onChange={e => {setPetName(e.target.value); setPetId('');}} style={getLockedFieldStyle()} />
                            </div>
                            <div style={{ flex: 1 }}>
                                ...
                                <select className="formInput" value={petType} disabled={isProfileLocked} onChange={e => { setPetType(e.target.value); setBreed(''); setCustomPetType(''); setCustomBreed(''); }} style={getLockedFieldStyle({ width: '100%', padding: '10px', height: '40px', boxSizing: 'border-box', marginBottom: petType === 'Other' ? '10px' : '0' })}>
                                    <option value="">Select type</option>
                                    <option value="Dog">Dog</option>
                                    <option value="Cat">Cat</option>
                                    <option value="Other">Other</option>
                                </select>
                                {petType === 'Other' && <input type="text" className="formInput" placeholder="Specify pet type" maxLength={30} value={customPetType} disabled={isProfileLocked} onChange={e => setCustomPetType(e.target.value)} style={getLockedFieldStyle({ width: '100%' })} />}
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                {petType === 'Other' ? (
                                    ...
                                    <>
                                        <select className="formInput" value={breed} onChange={e => { setBreed(e.target.value); setCustomBreed(''); }} disabled={!petType || isProfileLocked} style={getLockedFieldStyle({ width: '100%', padding: '10px', height: '40px', boxSizing: 'border-box', backgroundColor: (!petType || isProfileLocked) ? '#f9f9f9' : 'white', marginBottom: breed === 'Others' ? '10px' : '0' })}>
                                            <option value="">Select breed</option>
                                            {petType === 'Dog' && dogBreeds.map(b => <option key={b} value={b}>{b}</option>)}
                                            {petType === 'Cat' && catBreeds.map(b => <option key={b} value={b}>{b}</option>)}
                                        </select>
                                        {breed === 'Others' && <input type="text" className="formInput" placeholder="Specify breed" maxLength={30} value={customBreed} disabled={isProfileLocked} onChange={e => setCustomBreed(e.target.value)} style={getLockedFieldStyle({ width: '100%' })} />}
                                    </>
                                )}
                            </div>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="text" className="formInput" placeholder="e.g. Brown with white spots" maxLength={30} value={color} disabled={isProfileLocked} onChange={e => setColor(e.target.value)} style={getLockedFieldStyle()} />
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                                    ...
                                    <label style={{ fontSize: '11px', color: '#666', display: 'flex', alignItems: 'center', cursor: isProfileLocked ? 'not-allowed' : 'pointer', userSelect: 'none' }}>
                                        <input 
                                            type="checkbox" 
                                            checked={unknownDob} 
                                            ...
                                            style={{ marginRight: '5px', cursor: isProfileLocked ? 'not-allowed' : 'pointer' }} 
                                        />
                                        Unknown
                                    </label>
                                </div>
                                <input 
                                    ...
                                    disabled={unknownDob || isProfileLocked} 
                                    style={getLockedFieldStyle({ 
                                        backgroundColor: (unknownDob || isProfileLocked) ? '#f5f5f5' : 'white', 
                                        color: (unknownDob || isProfileLocked) ? '#aaa' : '#333',
                                        cursor: (unknownDob || isProfileLocked) ? 'not-allowed' : 'text' 
                                    })} 
                                />
                            </div>
                            <div style={{ flex: 1 }}>
                    ...
                    <div>
                        <div style={{ borderBottom: '2px solid #3d67ee', paddingBottom: '8px', marginBottom: '15px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: '600', color: '#555', margin: 0 }}>Medical Information</h3>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                            {medicalQuestionConfigs.map((questionConfig) => (
                                <div key={questionConfig.key}>
                                    <div style={{ fontSize: '12px', fontWeight: '600', color: '#555', marginBottom: '8px', textTransform: 'uppercase', lineHeight: '18px' }}>
                                        {questionConfig.question} <span style={{ color: '#d32f2f' }}>*</span>
                                    </div>
                                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                                        {[{ label: 'Yes', value: true }, { label: 'No', value: false }].map((option) => {
                                            const isSelected = medicalAnswers[questionConfig.key] === option.value;
                                            return (
                                                <button
                                                    key={`${questionConfig.key}-${option.label}`}
                                                    type="button"
                                                    onClick={() => setMedicalAnswers((prev) => ({ ...prev, [questionConfig.key]: option.value }))}
                                                    style={{
                                                        minWidth: '90px',
                                                        padding: '10px 16px',
                                                        borderRadius: '8px',
                                                        border: isSelected ? 'none' : '1px solid #d7deef',
                                    ...
                                    {questionConfig.key === 'medications72h' && medicalAnswers.medications72h === true && (
                                        <div style={{ marginTop: '10px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                                <label style={{ fontSize: '12px', fontWeight: '600', color: '#555' }}>{questionConfig.detailLabel} <span style={{ color: '#d32f2f' }}>*</span></label>
                                                <span style={{ fontSize: '11px', color: '#999' }}>{medicationDetails.length}/150</span>
                                            </div>
                                            <textarea
                                                className="formInput"
                                                rows={2}
                                                placeholder="List the medication or treatment given"
                                                maxLength={150}
                            ...
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <label style={{ fontSize: '12px', fontWeight: '600', color: '#555' }}>Additional Medical Notes</label>
                                    <span style={{ fontSize: '11px', color: '#999' }}>{medicalNotes.length}/200</span>
                                </div>
                                <textarea
                                    className="formInput"
                                    rows={3}
                                    placeholder="Optional notes about medical history, concerns, or reminders"
                                    maxLength={200}
                                    value={medicalNotes}
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {loadingSearch ? (
                                ...
                                filteredSearchList.map((pet, index) => {
                                    const owner = fetchedPatients.find(p => p.id === pet.owner_id);
                                    const ownerName = owner ? getPersonDisplayName(owner) : 'Unknown';
                                    return (
                                        <div 
...
const ConfirmationModal = ({ visible, onClose, onConfirm, title, message, confirmText = 'Yes', cancelText = 'No', confirmColor = '#3d67ee', type = 'info' }: any) => {
    ...
    const getIconProps = () => {
        switch(type) {
            ...
            case 'accept': return { icon: <IoCheckmarkCircleOutline size={40} color="#2e7d32" />, bg: '#e8f5e9' };
    ...
    const btnColor = type === 'cancel' ? '#d32f2f' : ((type === 'complete' || type === 'accept') ? '#2e7d32' : confirmColor);

    return (
        <div className="modalOverlay">
            <div className="modalContainer" style={{ width: '40%', maxWidth: '400px', padding: '0', display: 'flex', flexDirection: 'column', height: 'auto', maxHeight: '420px' }}>
                <div style={{ alignItems: 'center', paddingTop: '28px', paddingBottom: '18px', display: 'flex', justifyContent: 'center' }}>
                ...
                <div style={{ padding: '0 30px 8px', textAlign: 'center' }}>
                    <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#333', marginBottom: '10px' }}>{title}</h2>
                    <p style={{ fontSize: '16px', color: '#666', lineHeight: '22px', margin: 0 }}>{message}</p>
                </div>
                <div style={{ display: 'flex', padding: '18px 20px 20px', borderTop: '1px solid #f0f0f0', marginTop: '14px', gap: '15px' }}>
...
const DeclinePreferenceModal = ({ visible, onClose, appointment, requestDetails, onSubmit }: any) => {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            await onSubmit(generatedReason);
            onClose();
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="modalOverlay">
            <div className="modalContainer" style={{ width: '95%', maxWidth: '560px', display: 'flex', flexDirection: 'column', padding: '30px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#333', margin: 0 }}>Decline Preferred Schedule</h2>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                    <div style={{ backgroundColor: '#f8f9fa', padding: '16px', borderRadius: '10px', border: '1px solid #eee' }}>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: '#3d67ee', marginBottom: '10px' }}>Patient Preferred Schedule</div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                            <div>
                                <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Requested Date</div>
                                <div style={{ fontSize: '14px', fontWeight: '600', color: '#333' }}>{formatDateValue(requestDetails?.patient_preferred_date)}</div>
                            </div>
                            <div>
                                <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Requested Time</div>
                                <div style={{ fontSize: '14px', fontWeight: '600', color: '#333' }}>{formatTimeValue(requestDetails?.patient_preferred_time)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', fontSize: '14px', color: '#333' }}>
                            Reason for Declining <span style={{ color: '#d32f2f' }}>*</span>
                        </label>
                        <select
                            className="formInput"
                        ...
                        >
                            ...
                            {DECLINE_PREFERENCE_REASONS.map((reason) => (
                    ...
                    <div style={{ padding: '12px 14px', borderRadius: '8px', backgroundColor: '#eef6ff', border: '1px solid #cfe2ff', color: '#2c5da8', fontSize: '13px', fontWeight: '500' }}>
                        Email will be sent to: {appointmentEmail}
                    </div>
                </div>

                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '15px', marginTop: '24px', paddingTop: '18px', borderTop: '1px solid #eee' }}>
                    <button
                        onClick={onClose}
                        disabled={loading}
                        style={{ padding: '11px 24px', backgroundColor: '#f5f5f5', border: '1px solid #ddd', borderRadius: '8px', color: '#666', fontWeight: '600', cursor: 'pointer' }}
                    >
                        Cancel
                    </button>
                    <button
    ...
    const handleAssign = async () => {
        ...
        try {
            await onAssign(
                ...
                selectedDoctorId,
                appointment?.recordType || (appointment?.is_walk_in ? 'walkin' : 'appointment')
            );
            onClose();
        } catch (error) {
            ...
            <div className="modalContainer" style={{ width: '60%', maxWidth: '600px', display: 'flex', flexDirection: 'column', padding: '30px' }}>
                ...
                <div style={{ overflowY: 'auto', flex: 1 }}>
                    ...
                    <div style={{ backgroundColor: '#f9f9f9', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #eee' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}><span style={{ fontSize: '12px', color: '#666' }}>Date & Time:</span><span style={{ fontSize: '12px', fontWeight: '600' }}>{appointment?.date_display} {appointment?.time_display}</span></div>
                    </div>
                                ...
                                {doctors.map((doctor: any) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1 }} className="doctor-name-only">
...
export default function Schedule() {
    ...
    const [showDeclinePreferenceModal, setShowDeclinePreferenceModal] = useState(false);
    const [selectedAppointmentForPreferenceReview, setSelectedAppointmentForPreferenceReview] = useState<any>(null);
    const [selectedPreferenceRequest, setSelectedPreferenceRequest] = useState<any>(null);
    ...
    const handleAssignDoctor = async (appointmentId: any, doctorId: any, recordType: string = 'appointment') => {
        try {
            const result = await availabilityService.assignDoctor(appointmentId, doctorId, recordType);
            const updatedUserData = userData.map(appointment => {
                if ((appointment.dbId ?? appointment.id) === appointmentId && (appointment.recordType || 'appointment') === recordType) {
            ...
            if (selectedUser && (selectedUser.dbId ?? selectedUser.id) === appointmentId && (selectedUser.recordType || 'appointment') === recordType) {
                const doctor = doctors.find(d => (d.pk || d.id) === doctorId);
    ...
    const handleCancelWithReason = async (cancellationData: any) => {
        try {
            ...
            const result = await availabilityService.cancelAppointmentWithReason(
                ...
                fullCancelData,
                selectedAppointmentForCancel.recordType || (selectedAppointmentForCancel.is_walk_in ? 'walkin' : 'appointment')
            );
    ...
    const handleRescheduleSubmit = async (rescheduleData: any) => {
        try {
            ...
            const result = await availabilityService.createRescheduleRequest(
                selectedAppointmentForReschedule.dbId ?? selectedAppointmentForReschedule.id,
                fullRescheduleData,
                selectedAppointmentForReschedule.recordType || (selectedAppointmentForReschedule.is_walk_in ? 'walkin' : 'appointment')
            );
    ...
    const handleAcceptClientPreference = async (appointment: any, requestDetails: any) => {
        ...
        try {
            setLoading(true);
            const result = await availabilityService.reviewRescheduleRequest(requestDetails.request_id, 'accept');
            await loadAppointments();
            window.alert(
    ...
    const closeDeclinePreferenceModal = () => {
        setShowDeclinePreferenceModal(false);
        setSelectedAppointmentForPreferenceReview(null);
        setSelectedPreferenceRequest(null);
    };
    ...
    const handleDeclineClientPreference = (appointment: any, requestDetails: any) => {
        ...
        setSelectedAppointmentForPreferenceReview(appointment);
        setSelectedPreferenceRequest(requestDetails);
        setShowDeclinePreferenceModal(true);
    };
    ...
    const handleDeclineClientPreferenceSubmit = async (declineReason: string) => {
        ...
        try {
            setLoading(true);
            const result = await availabilityService.reviewRescheduleRequest(
    ...
    const handleAcceptAppointment = (appointment: any) => {
        ...
        setConfirmationAction(() => async () => {
            try {
                setLoading(true);
                const result = await availabilityService.updateAppointmentStatus(
                    appointment.dbId ?? appointment.id,
                    'confirmed',
                    appointment.recordType || (appointment.is_walk_in ? 'walkin' : 'appointment')
                );
    ...
    const loadAppointments = useCallback(async (options: { silent?: boolean; indicateRefresh?: boolean } = {}) => {
    const { silent = false, indicateRefresh = false } = options;
    if (!silent) setLoading(true);
    if (indicateRefresh) setRefreshingDetails(true);
    try {
        const rescheduleRequests = await availabilityService.getRescheduleRequests();
        const latestRequestByTarget = new Map<string, any>();

        (rescheduleRequests || []).forEach((requestItem: any) => {
            const mapKey = `${requestItem.target_type}-${requestItem.target_id}`;
            if (!latestRequestByTarget.has(mapKey)) {
                latestRequestByTarget.set(mapKey, requestItem);
            }
        });
        ...
        const medicalInformationRows = appointmentIds.length > 0
            ? await availabilityService.getMedicalInformationList(appointmentIds)
            : [];
        ...
        (medicalInformationRows || []).forEach((medicalInfoItem: any) => {
            ...
            const medicalInfoTargetId = medicalInfoItem?.target_id || medicalInfoItem?.walkin_id || medicalInfoItem?.appointment_id || '';
            const medicalInfoKey = `${medicalInfoRecordType}-${medicalInfoTargetId}`;
            if (medicalInfoTargetId && !medicalInformationByTarget.has(medicalInfoKey)) {
                medicalInformationByTarget.set(medicalInfoKey, medicalInfoItem);
            }
        });

        (doctorAccounts || []).forEach((doctorItem: any) => {
        ...
        // format regular appointments (your existing code)
        const formattedRegular = (appts || []).map(app => {
    ...
    } else if (app.walk_in_first_name) {
        ownerName = `${app.walk_in_first_name} ${app.walk_in_last_name || ''}`.trim();
    }

    const petName = pet ? pet.pet_name : (app.walk_in_pet_name || 'Unknown Pet');
    ...
    const latestRescheduleRequest = latestRequestByTarget.get(`appointment-${app.appointment_id || app.id}`) || null;
    const medicalInformation = medicalInformationByTarget.get(`appointment-${app.appointment_id || app.id}`) || null;
    const normalizedStatus = (app.status || 'scheduled').toLowerCase();
    const mappedReason = app.patient_reason || 'Not provided';
    const mappedRescheduleReason = latestRescheduleRequest?.reason || app.reschedule_reason || 'Not provided';
    const mappedEmail = owner?.email || 'Not provided';
    const mappedPhone = owner?.contact_number || 'Not provided';
    const mappedType = capitalizeFirstLetter(pet?.pet_species || 'Unknown');
    const mappedGender = capitalizeFirstLetter(pet?.pet_gender || 'Unknown');
    const mappedBreed = capitalizeFirstLetter(pet?.pet_breed || 'Unknown');
    const mappedDoctorId = app.assigned_doctor_id || app.doctor_id || null;
    const mappedDoctorRecord = mappedDoctorId ? doctorById.get(String(mappedDoctorId)) : null;
    ...
    return {
        ...
        dbId: app.appointment_id || app.id,
        recordType: 'appointment',
        ownerName, name: ownerName,
        petName,
        service: app.appointment_type || 'General',
        date_time: `${app.appointment_date} ${timeStr}`,
        date_display: app.appointment_date,
        ...
        rescheduleReason: mappedRescheduleReason,
        reschedule_reason: app.reschedule_reason || null,
        medicalInformation,
        medical_information: medicalInformation,
        latestRescheduleRequest,
        type: mappedType, petType: mappedType, pet_species: mappedType,
        breed: mappedBreed, petBreed: mappedBreed,
        gender: mappedGender, petGender: mappedGender,
        is_walk_in: false
    };
});

        // ← ADD THIS: format walk-ins to match same shape
        const formattedWalkIns = (walkIns || []).map(w => {
            const latestRescheduleRequest = latestRequestByTarget.get(`walkin-${w.walkin_id}`) || null;
            const medicalInformation = medicalInformationByTarget.get(`walkin-${w.walkin_id || w.id}`) || null;
            const normalizedStatus = (w.status || 'pending').toLowerCase();
            let timeStr = w.appointment_time || '';
            if (timeStr) {
                const [h, m] = timeStr.split(':');
                const startHours = parseInt(h, 10);
                const startAmPm = startHours >= 12 ? 'PM' : 'AM';
                ...
                const formatEndH = endHours % 12 || 12;
                timeStr = `${startTimeFormatted} - ${endHours % 12 || 12}:${m} ${endAmPm}`;
            }
            const mappedDoctorId = w.assigned_doctor_id || w.doctor_id || null;
            const mappedDoctorRecord = mappedDoctorId ? doctorById.get(String(mappedDoctorId)) : null;
            const mappedDoctorName = mappedDoctorRecord
                ? (getPersonDisplayName(mappedDoctorRecord) || 'Not Assigned')
                : 'Not Assigned';
            ...
            return {
                ...
                dbId: w.walkin_id,
                recordType: 'walkin',
                ownerName: `${w.first_name} ${w.last_name}`.trim(),
                name: `${w.first_name} ${w.last_name}`.trim(),
                petName: w.pet_name || 'Unknown Pet',
                service: w.appointment_type || 'General',
                date_time: `${w.appointment_date} ${timeStr}`,
                date_display: w.appointment_date,
                time_display: timeStr,
                date_only: w.appointment_date,
                status: normalizedStatus,
                displayStatus: getDisplayAppointmentStatus(normalizedStatus, latestRescheduleRequest),
                doctor: mappedDoctorName,
                assignedDoctor: mappedDoctorId,
                ...
                contact_number: w.contact_number || 'Not provided',
                reasonForVisit: w.patient_reason || 'Not provided',
                patient_reason: w.patient_reason || 'Not provided',
                rescheduleReason: latestRescheduleRequest?.reason || w.reschedule_reason || 'Not provided',
                reschedule_reason: w.reschedule_reason || null,
                medicalInformation,
                medical_information: medicalInformation,
                latestRescheduleRequest,
                type: capitalizeFirstLetter(w.pet_species || 'Unknown'),
                petType: capitalizeFirstLetter(w.pet_species || 'Unknown'),
                breed: capitalizeFirstLetter(w.pet_breed || 'Unknown'),
                gender: capitalizeFirstLetter(w.pet_gender || 'Unknown'),
                is_walk_in: true,   // ← flag so you know it's a walk-in
                raw_data: w
            };
        });

        // ← MERGE BOTH LISTS
        const formattedData = [...(formattedRegular || []), ...formattedWalkIns]
        .sort((a: any, b: any) => (a.date_only > b.date_only ? 1 : -1));

        setUserData(formattedData);
        
        const booked: any = {};
        formattedData.forEach((app: any) => {
            if (app.date_only) booked[app.date_only] = true;
        });
        setBookedDates(booked);

        const currentSelectedUser = selectedUserRef.current;
        if (currentSelectedUser) {
            const refreshedSelection = formattedData.find((item: any) =>
                (item.dbId ?? item.id) === (currentSelectedUser.dbId ?? currentSelectedUser.id) &&
                (item.recordType || 'appointment') === (currentSelectedUser.recordType || 'appointment')
            );

            if (refreshedSelection) {
                setSelectedUser(refreshedSelection);
                setSelectedAppointment(refreshedSelection);
            }
        }

    } catch (error) {
        console.error("Error loading appointments:", error);
        if (!silent || indicateRefresh) {
    ...
    const loadDoctors = async () => {
        try {
            ...
            const formattedDoctors = doctorsList.map((doctor: any) => ({
                ...
                pk: doctor.pk,
                fullName: getPersonDisplayName(doctor),
                name: getPersonDisplayName(doctor),
                role: doctor.role || 'Veterinarian',
                department: doctor.department || 'General',
                userImage: doctor.userImage || doctor.userimage || doctor.employee_image,
            }));
    ...
    const handleSubmitAppointment = async (appointmentData: any) => {
        try {
            ...
            const createdAppointment = await availabilityService.createAppointment(submissionData);
            const createdRecordType = createdAppointment?.recordType || 'appointment';
            const createdAppointmentId =
                createdAppointment?.target_id ??
                createdAppointment?.walkin_id ??
                createdAppointment?.appointment_id ??
                createdAppointment?.data?.[0]?.appointment_id ??
                createdAppointment?.data?.[0]?.walkin_id ??
                createdAppointment?.data?.[0]?.id;
            let medicalInformationNote = '';
            const bookingEmailNote =
                createdAppointment?.emailSent === false
                    ? 'Booking confirmation email could not be sent.'
                    : '';

            if (appointmentData?.medical_information) {
                if (createdAppointmentId) {
                    try {
                        await availabilityService.saveMedicalInformation(
                            createdAppointmentId,
                            appointmentData.medical_information,
                            createdRecordType === 'walkin' ? 'walkin' : 'appointment'
                        );
                    } catch (medicalInformationError: any) {
                        medicalInformationNote = medicalInformationError?.message || 'Medical information could not be saved.';
                    }
                } else {
                    medicalInformationNote = 'Medical information could not be linked because the appointment ID was not returned.';
                }
            }
    ...
    }, [currentView, selectedUser, loadAppointments]);
        ...
        <div className="biContainer">
                            ...
                            <div className="doctors-scroll-container" style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                {doctors.map((doctor, index) => (
                                    ...
                                    >
                                        ...
                                        <div className="doctor-name-only">
            ...
            {showDeclinePreferenceModal && (
                <DeclinePreferenceModal
                    ...
                    appointment={selectedAppointmentForPreferenceReview}
                    requestDetails={selectedPreferenceRequest}
                    onSubmit={handleDeclineClientPreferenceSubmit}
                />
            )}
