    ...
    useEffect(() => {
        if (visible) {
            const loadDatabase = async () => {
                ...
                try {
                    const response: any = await apiService.getAdminAppointmentSearchData();
                    setFetchedPets(response?.pets || []);
                    setFetchedPatients(response?.patients || []);
                } catch (error) {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1 }} className="doctor-name-only">
                                                ...
                                                <div style={{ fontSize: '12px', color: '#666', marginTop: '2px' }}>{doctor.role || 'Veterinarian'}</div>
                                            </div>
...
export default function Schedule() {
        ...
        try {
            ...
            const formattedDoctors = doctorsList
                .filter((doctor: any) => {
                    ...
                    return normalizedRole === 'veterinarian' || normalizedRole === 'vet';
                })
                .map((doctor: any) => ({
    ...
    const handleSubmitAppointment = async (appointmentData: any) => {
        try {
            ...
            const createdAppointment = await apiService.createAdminAppointment(submissionData);
                ...
                if (createdAppointmentId) {
                    try {
                        const medPayload = {
                            ...
                            record_type: createdRecordType === 'walkin' ? 'walkin' : 'appointment',
                            ...(createdRecordType === 'walkin' ? { walkin_id: createdAppointmentId } : { appointment_id: createdAppointmentId })
                        };
                        await apiService.saveMedicalInformation(medPayload);
    ...
    const loadDayAvailability = async () => {
        try {
            const dayData = await apiService.getDayAvailability();
